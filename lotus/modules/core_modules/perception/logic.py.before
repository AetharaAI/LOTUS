...
class PerceptionModule(BaseModule):
    ...
    def __init__(self, name: str, metadata: Dict, message_bus: Any, config: Any, logger: logging.Logger):
        ...
        self.last_screen_image: Optional[Image.Image] = None # Store PIL image for diffing
        
        # Configuration for perception sources
        ...
        # Note: OCR setup (e.g., pytesseract) would go here and its dependencies checked.
            ...
            try:
                ...
                if current_content != self.last_clipboard_content:
                    ...
                    await self.publish("perception.raw.clipboard_changed", {
                        "content": display_content, # Publish truncated for performance, orchestrator can ask for full if needed
                        "length": content_length,
                        "timestamp": time.time(),
                        "context": {"active_file": None, "active_directory": None} # Placeholder context
                    })
        ...
        with mss.mss() as sct:
            while True:
                try:
                    ...
                    screenshot_bytes = sct.grab(sct.monitors[0]) # First monitor
                    ...
                    if self.screen_ocr_enabled:
                        try:
                            import pytesseract # Defer import for optional dependency
                            ...
                            self.screen_ocr_enabled = False # Disable for session
                        except Exception as ocr_e:
                            self.logger.error(f"Error during OCR: {ocr_e}", exc_info=True)
                    
                    if current_hash != self.last_screen_hash or change_percentage > 0.01: # Small threshold for noise
                        ...
                        # In a real system, you'd store the image bytes to disk/S3 and pass a reference
                        # For now, pass a dummy ref or base64 encode small images (be careful with size!)
                        # The Orchestrator can then decide to pull the full image if needed.
                        await self.publish("perception.raw.screen_update", {
                            "image_hash": current_hash,
                            "change_percentage": change_percentage,
                            "text_content": text_content, # Extracted OCR text
                            "image_ref": f"screen_capture_{int(time.time())}.png", # Placeholder for actual storage
                            "timestamp": time.time(),
                            "context": {}
                        })
    ...
    def _calculate_image_hash(self, image: Image.Image) -> str:
        ...
        # A full pHash library would be better, but for quick diff:
        # Resize, convert to grayscale, calculate hash
    ...
    def _calculate_image_diff(self, img1: Image.Image, img2: Image.Image) -> float:
        ...
        if img1.size != img2.size:
            return 1.0 # Treat different sizes as 100% different
        
        diff_pixels = 0
        total_pixels = img1.width * img1.height
        
        # Simple pixel-by-pixel diff (can be slow for large images)
    ...
    # --- User Input Handlers (Now just publish raw) ---

    @on_event("perception.user_input") # Still listen for user_input from CLI/etc.
    async def on_user_input(self, event: Dict) -> None:
        """
        Handles user text input and publishes it as a raw perception event
        for the ContextOrchestrator.
        """
        ...
        # Publish to the raw channel for the ContextOrchestrator
    ...
    async def on_voice_input(self, event: Dict) -> None:
        """
        ...
        await self.publish("perception.raw.user_input", { # Treat voice as user input for now
            "text": transcript,
    ...
    async def shutdown(self):
        ...
        if self.screen_monitor_task:
            ...
            try:
                await self.screen_monitor_task
            except asyncio.CancelledError:
                pass
